...
def visualize2d(img, predictions, labels=None, filename=None, palm=None, evaluate=False):
    ...
    plot_pose2d(img, predictions, 0, fig_config, plot_id, 'Predicted 2D pose', center=palm, dataset_name='h3o')
    plot_id += 1
...
# rgb_errors = []
files = ["%03d.png" % i for i in range(50)]
#files = ["{:06d}".format(i)+".png" for i in range(0,676,50)]
for filename in files:
    ...
    # Read the image
    image = cv2.imread("./violin/"+filename)#"./handBkgd/"+filename)
    ...
    predictions, img, palm, labels = prepare_data_for_evaluation({}, outputs, img, keys, device, args.split)
    ...
    if args.visualize: 
        ...
        if (num_classes == 2 and 1 in predictions['labels']) or (num_classes == 4 and set([1, 2, 3]).issubset(predictions['labels'])):
            visualize2d(img, predictions, predictions, filename=name, palm=palm, evaluate=evaluate)
        else:
            cv2.imwrite("./results/"+filename, cv2.cvtColor(img, cv2.COLOR_RGB2BGR))
